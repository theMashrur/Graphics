...
in vec2 vertexOutTextureCoordinates;
...
void main() {
  ...
  // Get texture colour
  vec4 texColor = texture(sampler, vertexOutTextureCoordinates);
  
  // Calculate fragment colour
  fragColor = lightIntensity * (ambientColor + diffuseTerm * diffuseColor * texColor + specularTerm * specularColor);
}